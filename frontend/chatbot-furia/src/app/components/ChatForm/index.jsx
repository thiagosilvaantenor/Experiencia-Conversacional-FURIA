  ...
  const enviarOpcao = async (valor) => {
   ...
   switch(valor){
    ...
    default:
      ...
      setCarregando(false);
      setMensagens(prev => [...prev, { texto: data.mensagem, remetente: 'bot' }]);
    }
    
  }

  //Caso opção seja = 1
  const verificaLive = (data) => {
    //Recebe o json(data) e verifica se alguem está emLive
    if (data.emLive) {
      data.emLive.forEach(jogador => {
        setMensagens(prev => [
          ...prev,
          {
      ...
      setMensagens(prev => [
        ...prev,
        {
          texto:data.mensagem, remetente: "bot"
        }
      ]);
        ...
        {mensagens.map((msg, idx) => (
          <div key={idx} className={`flex ${msg.remetente === 'bot' ? 'justify-start' : 'justify-end'}`}>
            <div className={`text-black text-stro shadow-md/30 border-black p-4 rounded-2xl max-w-sm break-words whitespace-pre-wrap
              ${msg.remetente === 'bot' ? 'bg-blue-500' : 'bg-yellow-200'}`}>
              {msg.texto}
           </div>
          </div>
        ))}
